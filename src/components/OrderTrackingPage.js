...
const capitalizeFirstLetter = (string) => {
  ...
  <div className="flex justify-between text-gray-700 mb-6">
    ...
    <span className="font-medium">{capitalizeFirstLetter(orderDetails.branch)} Branch</span> {/* Ensure the branch name is capitalized */}
  </div>
</div>
...
<div className="border-t pb-10 pt-4">
  <div className="flex justify-end flex-col items-end">
    ...
    <p className="text-xl font-bold   rounded-md p-2 text-right">
      ...
      🌟 Hi there! Please refresh for the latest order updates. Orders usually take about 5 to 10 minutes to prepare. If you have any questions or need to cancel your order, feel free to contact us.  </div>
      <hr className="pb-10" />
      ...
      <ol className="relative border-l border-gray-200">
        {statuses.map((status, index) => (
          <li key={status} className={`mb-10 ml-6 ${index <= currentStatusIndex ? 'text-blue-500' : 'text-gray-500'}`}>
            ...
            <h3 className={`flex items-center mb-1 text-lg font-semibold ${index <= currentStatusIndex ? 'text-gray-900' : 'text-gray-500'}`}>{status}</h3>
            <time className="block mb-2 text-sm font-normal leading-none text-gray-400"></time>
          </li>
        ))}
      </ol>
    </div>
